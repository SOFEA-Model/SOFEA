...
{
    return httpHeaders_.find(header) != httpHeaders_.end();
}

void CurlEasy::removeHttpHeader(const QString &header)
{
    httpHeaders_.erase(header);
    httpHeadersWereSet_ = true;
}

QString CurlEasy::httpHeader(const QString &header) const
{
    auto it = httpHeaders_.find(header);
    return (it != httpHeaders_.end()) ? QUrl::fromPercentEncoding(it->second) : QString();
}
...
{
    auto it = httpHeaders_.find(header);
    return (it != httpHeaders_.end()) ? it->second : QByteArray();
}
