...
// proj_internal.h
extern std::string PROJ_DLL pj_context_get_user_writable_directory(PJ_CONTEXT *ctx, bool create);
...
{
    proj_context_set_database_path(Context::instance(), path.c_str(), nullptr, nullptr);
}

std::string Context::databasePath()
{
    return proj_context_get_database_path(Context::instance());
}

void Context::setCacheDirectory(const std::string& path)
{
    pj_context_set_user_writable_directory(Context::instance(), path.c_str());
}

std::string Context::cacheDirectory()
{
    return pj_context_get_user_writable_directory(Context::instance(), false);
}
      ...
      vDatumCode_(vDatumCode), vUnitsCode_(vUnitsCode)
{
    thread_local auto dbContext = io::DatabaseContext::create(
        Context::databasePath(), std::vector<std::string>(), Context::instance());
...
Transform::Transform(const std::string& from, const std::string& to, const Area& bbox)
{
    ...
    pdata_ = std::shared_ptr<PJconsts>(
        proj_create_crs_to_crs(Context::instance(), from.c_str(), to.c_str(), area_.get()),
