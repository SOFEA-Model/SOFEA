...
bool GeosOp::isValid(const QPolygonF& polygon)
{
    GEOSGeometry *geom = polygonToGeos(polygon);
    char flag = GEOSisValid(geom);
    return flag == 1 ? true : false;
}

bool overlaps(const QPolygonF& polygon1, const QPolygonF& polygon2)
{
    GEOSGeometry *geom1 = polygonToGeos(polygon1);
    GEOSGeometry *geom2 = polygonToGeos(polygon2);
    char flag = GEOSOverlaps(geom1, geom2);
    return flag == 1 ? true : false;
}
